...
def eval_predictions(eval_data_path, predictions, labels, output_dir):
    ...
    def in_dataset(pred, label):
        ...
        return [
            any(
                any(
                    isinstance(p, str) and p.lower() == str(item).lower()
                    for p in pred_sublist
                )
                for item in label_sublist
            )
            for pred_sublist, label_sublist in zip(pred, label)
        ], None
    ...
    for condition in conditions:
        ...
        for k in [1, 3, 5, 10]:
            ...
            pred_in_data, _ = in_dataset(modified_predictions, modified_labels)
            results[condition_name]['percentage_found_in_dataset'] = (sum(pred_in_data) / len(pred_in_data)) * 100
